...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config: LLaMAConfig, block_idx: int) -> None:
        ...
        self.sparse_attn = SparseAttention(block_size=config.block_size)
    ...
    def forward(
        ...
        # y = F.scaled_dot_product_attention(
            # q, k, v, attn_mask=mask, dropout_p=0.0)  # (B, nh, T, hs)

        # 변경 부분
        y = self.sparse_attn(q, k, v)
        ...
        if self.block_idx >= self.adapter_start_layer:
            if adapter_kv_cache is not None:
                ...
                prefix = self.adapter_wte.weight.reshape(
                    1, self.adapter_prompt_length, self.n_embd)
                aT = prefix.size(1)
                # (1, aT, 3 * C) --> 3 * (1, aT, C)
                _, ak, av = self.c_attn(prefix).split(self.n_embd, dim=2)
                ak = ak.view(1, aT, self.n_head, head_size).repeat(
                    B, 1, 1, 1).transpose(1, 2)  # (B, nh, aT, hs)
                av = av.view(1, aT, self.n_head, head_size).repeat(
                    B, 1, 1, 1).transpose(1, 2)  # (B, nh, aT, hs)
            ...
            amask = torch.ones(
                q.shape[-2], ak.shape[-2], dtype=torch.bool, device=x.device)  # (T, aT)
        ...
        if name in state_dict:
            ...
            if len(tensor.shape) < 4:
                ...
                state_dict[name] = tensor.reshape(
                    1, 1, 1, 1).repeat(1, self.n_head, 1, 1)
            else:
...
class Block(nn.Module):
    ...
    def forward(
        ...
        h, new_kv_cache, new_adapter_kv_cache = self.attn(
            self.rms_1(
                x), rope, mask, max_seq_length, input_pos, kv_cache, adapter_kv_cache
        )
        x = x + h
    ...
    def forward(
        ...
        if self.rope_cache is None:
            self.rope_cache = self.build_rope_cache(
                idx)  # (block_size, head_size / 2, 2)
        if self.mask_cache is None:
            self.mask_cache = self.build_mask_cache(
                idx)  # (1, 1, block_size, block_size)
        ...
        if input_pos is None:  # proxy for use_cache=False
            ...
            for i, block in enumerate(self.transformer.h):
                x, self.kv_caches[i], self.adapter_kv_caches[i] = block(
                    x, rope, mask, max_seq_length, input_pos, self.kv_caches[
                        i], self.adapter_kv_caches[i]
                )
