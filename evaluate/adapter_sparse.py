...
def main(
    ...
    *,
    ...
    adapter_path: Path = Path("out/final/c2/adapter_v1_base/lit-llama-adapter-finetuned.pth"), # 수정
    checkpoint_path: Path = Path("checkpoints/open-llama/7B/pytorch_model-00002-of-00002.bin"), # 수정
    tokenizer_path: Path = Path("checkpoints/open-llama/7B/tokenizer.model"), # 수정
    dtype: str = "float32",
    ...
    with lazy_load(checkpoint_path) as pretrained_checkpoint, lazy_load(adapter_path) as adapter_checkpoint:
        ...
        # 디버깅 위해 추가
        # print(pretrained_checkpoint.keys())
